...
with tab2:
    ...
    # Show pairplot for a sample of numeric features (colorful, interactive)
...
with tab6:
    ...
    model_type = st.selectbox("Select model type:", [
        ...
        "Federated Averaging (Simulation - if available)",
        "XGBoost (if in notebook)"
    ])
    ...
    if model_type == "Random Forest (Classical)":
        ...
        if train_model and X_train is not None:
    ...
    elif model_type == "XGBoost (if in notebook)":
        try:
            from xgboost import XGBClassifier
            n_estimators = st.slider("Number of trees (n_estimators):", 50, 300, 120, step=10, key="xgb_nest")
            xgb_model = XGBClassifier(n_estimators=n_estimators, random_state=42, use_label_encoder=False, eval_metric="mlogloss")
            train_model = st.button("Train XGBoost", key="train_xgb")
            if train_model and X_train is not None:
                xgb_model.fit(X_train, y_train)
                y_pred = xgb_model.predict(X_test)
                st.session_state['model'] = xgb_model
                st.session_state['y_pred'] = y_pred
                st.success("🎉 XGBoost trained and predictions saved!")
                st.balloons()
                st.write("**Feature importances:**")
            ...
            st.warning("XGBoost not installed. Add `xgboost` to requirements.txt to use.")
    elif model_type == "Federated Averaging (Simulation - if available)":
        st.info("Federated learning logic can be added here as per your notebook. (Upload your federated code section if needed!)")
        st.warning("**This section is a placeholder unless federated logic/code is provided.**")
        # Simulate output, if code is in notebook, paste here.
with tab7:
    ...
    if y_test is None or y_pred is None:
        st.warning("Please train a model first (see previous tab).")
    else:
...
with tab9:
    ...
    if y_pred is not None and X_test is not None:
        ...
        result_df["True"] = st.session_state['y_test'].values
